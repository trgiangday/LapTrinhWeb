from flask import Flask, render_template, url_for, session, redirect, jsonify , request, flash, get_flashed_messages, g
import sqlite3
import re
...
@app.route('/show_exams_page')
def show_exams_page():
    conn = sqlite3.connect('database.db')
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM exams")
    exams = cursor.fetchall()
    exams_list = [{'id': exam[0], 'name': exam[1], 'subject': exam[2], 'grade': exam[3], 'duration': exam[4], 'creator_username': exam[5]} for exam in exams]
    conn.close()
...
def search_exams():
    ...
    if search_term:
        query += " AND (name LIKE ? OR creator_username LIKE ?)"
...
# Route callback sau khi người dùng xác thực với Google
@app.route('/callback')
def callback():
    ...
    fullname = user_info.get('name', 'Unknown')
    ...
    if user:
        ...
        session['username'] = user['username']
        session['is_admin'] = 0  # Giả định người dùng Google không phải admin
        session['is_teacher'] = 0
    else:
        # Nếu người dùng chưa tồn tại, tạo tài khoản mới
        # Tạo username duy nhất nếu username đã tồn tại
        ...
        # Đăng ký người dùng mới (không cần mật khẩu vì đăng nhập qua Google)
        db.execute(
            'INSERT INTO users (fullname, phone, gender, username, email, password, is_admin, is_teacher) VALUES (?, ?, ?, ?, ?, ?, 0, 0)',
            (fullname, '', 'unknown', username, email, '')
        )
        conn.commit()
