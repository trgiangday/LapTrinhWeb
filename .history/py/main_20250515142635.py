...
def forgot_password():
    if request.method == 'POST':
        ...
        if not user:
            ...
            password = "your_app_password"  # Thay bằng mật khẩu ứng dụng (App Password) của Gmail
...
def reset_password(token):
    # Logic để xác minh token và đặt lại mật khẩu (cần triển khai thêm)
    if request.method == 'POST':
        new_password = request.form['password']
        # Xác minh token và cập nhật mật khẩu (giả định token hợp lệ)
        conn = get_db()
        db = conn.cursor()
